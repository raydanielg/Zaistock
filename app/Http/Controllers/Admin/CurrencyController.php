...
class CurrencyController extends Controller
{
    ...
    public function index(Request $request)
    {
                ...
                ->select('id', 'currency_code', 'current_currency', 'symbol', 'currency_placement');
                    return datatables($currencies)
                        ...
                        ->editColumn('currency_code', function ($data) {
                            ...
                            if ($data->current_currency == ACTIVE) {
                        ...
                        ->addColumn('action', function ($data) {
                            return '<ul class="d-flex align-items-center cg-5 justify-content-end">
                                        <li class="align-items-center d-flex gap-2">
                                            <button onclick="getEditModal(\'' . route('admin.setting.currency.edit', $data->id) . '\'' . ', \'#edit-modal\')" class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white" data-bs-toggle="modal" data-bs-target="#alumniPhoneNo">
                                            ...
                                            <button onclick="deleteItem(\'' . route('admin.setting.currency.delete', $data->id) . '\', \'currencyDataTable\')" class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white" title="Delete">
        ...
        $data['pageTitle'] = 'Currency Setting';
    ...
    public function edit($id)
    {
        ...
        return view('admin.setting.currency.edit', $data);
    }
    ...
    public function store(CurrencyRequest $request)
    {
        ...
        if ($request->current_currency)
        {
            ...
            Currency::where('id', '!=', $currency->id)->update(['current_currency' => DISABLE]);
        }
            DB::commit();
    ...
    public function delete($id)
    {
        try {
            ...
            if ($currency->current_currency == ACTIVE) {
                throw new Exception('This is the current currency and cannot be deleted.');
            }
            if (Gateway::where('gateway_currency', $currency->currency_code)->exists()) {
                throw new Exception('This currency is already used by a gateway.');
            }
