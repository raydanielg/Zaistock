<table class="table zTable zTable-last-item-right commonDataTable">
    ...
    <tr>
        ...
        <th><div class="text-nowrap">{{__('Device Limit')}}</div></th>
        <th><div class="text-nowrap">{{__('Download Limit')}}</div></th>
        <th><div>{{__('Tax')}}</div></th>
        ...
        <tr class="removable-item">
            ...
            <td>
                <div class="finance-table-inner-item my-2">
                    <span class="fw-bold mr-1">{{ __('Monthly price') }}:</span> {{ getCurrencySymbol() }}{{ number_format($plan->monthly_price, 2) }}
                </div>
                 <div class="finance-table-inner-item my-2">
                    <span class="fw-bold mr-1">{{ __('Yearly price') }}:</span> {{ getCurrencySymbol() }}{{ number_format($plan->yearly_price, 2) }}
                </div>
            ...
            <td>
                <div class="finance-table-inner-item my-2">
                    <span class="fw-bold mr-1">{{ __('Type') }}:</span> {{ $plan->download_limit_type == PLAN_DOWNLOAD_LIMIT_TYPE_UNLIMITED ? __('Unlimited'):__('Limited') }}
                </div>

                @if($plan->download_limit_type == 2)
                <div class="finance-table-inner-item my-2">
                    <span class="fw-bold mr-1">{{ __('Download Limit') }}: </span>{{ $plan->download_limit }}
                </div>
                @endif
            </td>
            <td>
                @if(@$plan->tax->percentage)
                    {{ @$plan->tax->name . '(' . @$plan->tax->percentage . '%)' }}
                @else
                    -
                @endif
            </td>
            <td>
                <span id="hidden_id" class="d-none">{{$plan->id}}</span>
                <select name="status" class="status form-control">
                    <option value="1" {{ $plan->status == ACTIVE ? 'selected':'' }} >{{ __('Active') }}</option>
                    <option value="0" {{ $plan->status == DISABLE ? 'selected':'' }} >{{ __('Disable') }}</option>
                </select>
            </td>
            ...
            <td>
                <div class="d-flex justify-content-end align-items-center g-10">
                    <a href="{{route('admin.plan.edit', [$plan->uuid])}}" title="Edit" class="p-0 bg-transparent w-30 h-30 bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center">
                    ...
                    <button class="p-0 bg-transparent w-30 h-30 bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center deleteItem" data-formid="delete_row_form_{{$plan->uuid}}">
                    ...
                    <form action="{{ route('admin.plan.destroy', $plan->uuid) }}" method="post" id="delete_row_form_{{ $plan->uuid }}">
                        {{ method_field('DELETE') }}
