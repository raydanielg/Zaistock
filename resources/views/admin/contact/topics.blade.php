...
@section('content')
    ...
    <div class="p-sm-30 p-15">
        ...
        <div class="d-flex justify-content-end align-items-center g-10 pb-24">
            <button class="border-0 bd-ra-12 bg-primary py-13 px-25 fs-16 fw-600 lh-19 text-white" type="button" data-bs-toggle="modal" data-bs-target="#add-todo-modal"><i class="fa fa-plus"></i> {{ __('Add Topic') }}</button>
        </div>
        ...
        <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30 p-15">
            <table class="table zTable zTable-last-item-right commonDataTable">
                ...
                <tbody>
                @foreach($contactUsTopics as $topic)
                    <tr class="removable-item">
                        ...
                        <td>
                            <div class="d-flex justify-content-end align-items-center g-5">
                                <a class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white edit" data-item="{{ $topic }}"
                                   data-updateurl="{{route('admin.contact.topic.update', [$topic->id])}}"
                                   data-toggle="tooltip" title="Edit">
                                    <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M11.8067 3.19354C12.0667 2.93354 12.0667 2.5002 11.8067 2.25354L10.2467 0.693535C10 0.433535 9.56667 0.433535 9.30667 0.693535L8.08 1.91354L10.58 4.41354M0 10.0002V12.5002H2.5L9.87333 5.1202L7.37333 2.6202L0 10.0002Z" fill="#5D697A"></path>
                                    </svg>
                                </a>

                                <button class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white">
                                    <span data-formid="delete_row_form_{{ $topic->id }}" class="deleteItem" title="Delete">
                                ...
                                <form action="{{ route('admin.contact.topic.destroy', $topic->id) }}" method="post" id="delete_row_form_{{ $topic->id }}">
                                    {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bd-c-stroke-color bd-ra-12 py-25 px-20">
                <div class="d-flex justify-content-between align-items-center g-10 pb-20 mb-20 bd-b-one bd-c-stroke">
                    <h5 class="fs-18 fw-500 lh-28 text-primary-dark-text">{{ __('Add Contact Us Help Topic') }}</h5>
                ...
                <form action="{{route('admin.contact.topic.store')}}" method="post" autocomplete="off">
                    @csrf
                    ...
                    <div class="d-flex justify-content-end align-items-center g-10 bd-t-one bd-c-stroke pt-15">
    ...
    <div class="modal fade edit_modal" id="add-todo-modal" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bd-c-stroke-color bd-ra-12 py-25 px-20">
                <div class="d-flex justify-content-between align-items-center g-10 pb-20 mb-20 bd-b-one bd-c-stroke">
                    <h5 class="fs-18 fw-500 lh-28 text-primary-dark-text">{{ __('Edit Contact Us Topic') }}</h5>
                    <button type="button" class="w-30 h-30 rounded-circle d-flex justify-content-center align-items-center bd-one bd-c-stroke-color bg-transparent" data-bs-dismiss="modal" aria-label="Close"><i class="fa-solid fa-times"></i></button>
                </div>
                <form action="" id="updateEditModal" method="post">
                    ...
                    <div class="bd-t-one bd-c-stroke d-flex justify-content-end align-items-center g-10">
...
@push('script')
    <script src="{{ asset('admin/js/custom/topics.js') }}"></script>
@endpush
