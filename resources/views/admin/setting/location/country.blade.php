...
@section('content')
    ...
    <div class="p-sm-30 p-15">
        ...
        <div class="row rg-24">
            ...
            <div class="col-xl-9">
                <div class="d-flex flex-wrap justify-content-between align-items-center g-10 flex-wrap pb-30">
                    <h2 class="fs-18 fw-600 lh-20 text-primary-dark-text">{{ __('Country') }}</h2>
                    <button class="border-0 bg-primary py-8 px-26 bd-ra-8 fs-15 fw-600 lh-25 text-white d-inline-flex align-items-center g-10" type="button" data-bs-toggle="modal" data-bs-target="#add-todo-modal">
                ...
                <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30 p-15">
                    <table class="table zTable zTable-last-item-right commonDataTable">
                        ...
                        <tr>
                            ...
                            <th><div class="text-nowrap">{{__('Short Name')}}</div></th>
                            <th><div class="text-nowrap">{{__('Phone Code')}}</div></th>
                            <th><div>{{__('Continent')}}</div></th>
                            ...
                            <tr class="removable-item">
                                ...
                                <td>
                                    <div class="d-flex justify-content-end align-items-center g-10">
                                        <a href="{{ route('admin.setting.location.country.edit', [$country->id, $country->slug]) }}" class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white edit" data-toggle="tooltip" title="Edit">
                                        ...
                                        <button class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white deleteItem" data-formid="delete_row_form_{{$country->id}}">
                                        ...
                                        <form action="{{ route('admin.setting.location.country.delete', $country->id) }}" method="post" id="delete_row_form_{{ $country->id }}">
                                            {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content bd-c-stroke-color bd-ra-12 py-25 px-20">
                <div class="d-flex justify-content-between align-items-center g-10 pb-20 mb-20 bd-b-one bd-c-stroke">
                    <h5 class="fs-18 fw-500 lh-28 text-primary-dark-text">{{ __('Add Country') }}</h5>
                ...
                <form action="{{ route('admin.setting.location.country.store') }}" method="post">
                    @csrf
                    <div class="pb-24">
                        <div class="row rg-24">
                            <div class="col-md-6">
                                    <label class="zForm-label" for="country_name">{{ __('Name') }}</label>
                                    <input class="zForm-control" type="text" name="country_name" id="country_name" placeholder="Type country name" value="{{ old('country_name') }}" required>
                                    @if ($errors->has('country_name'))
                                        <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> {{ $errors->first('country_name') }}</span>
                                    @endif
                            </div>
                            <div class="col-md-6">
                                    <label class="zForm-label" for="country_name">{{ __('Short name') }}</label>
                                    <input class="zForm-control" type="text" name="short_name" id="short_name" placeholder="Type short name" value="{{ old('short_name') }}" required>
                                    @if ($errors->has('short_name'))
                                        <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> {{ $errors->first('short_name') }}</span>
                                    @endif
                            </div>
                            <div class="col-md-6">
                                    <label class="zForm-label" for="phonecode">{{ __('Phone Code') }}</label>
                                    <input class="zForm-control" type="text" name="phonecode" id="phonecode" placeholder="Type phone code" value="{{ old('phonecode') }}" required>
                                    @if ($errors->has('phonecode'))
                                        <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> {{ $errors->first('phonecode') }}</span>
                                    @endif
                            </div>
                            <div class="col-md-6">
                                    <label class="zForm-label" for="continent">{{ __('Continent') }}</label>
                                    <input class="zForm-control" type="text" name="continent" id="continent" placeholder="Type continent" value="{{ old('continent') }}" required>
