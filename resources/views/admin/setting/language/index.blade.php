...
@section('content')
    ...
    <div class="p-sm-30 p-15">
        <div class="d-flex align-items-center justify-content-between flex-wrap g-15 pb-26">
            <h2 class="fs-24 fw-600 lh-29 text-primary-dark-text">{{ __('Settings') }}</h2>
            <div class="">
        ...
        <div class="row rg-24">
            ...
            <div class="col-xl-9">
                <div class="d-flex justify-content-between align-items-center g-10 flex-wrap pb-24">
                    ...
                    <a href="{{ route('admin.setting.language.create') }}" class="border-0 bg-primary py-8 px-26 bd-ra-8 fs-15 fw-600 lh-25 text-white"><i class="fa fa-plus"></i> {{ __('Add Language') }} </a>
                </div>
                ...
                <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30 p-15">
                    <table class="table zTable zTable-last-item-right commonDataTable">
                        <thead>
                            <tr>
                                <th><div>{{ __('SL') }}</div></th>
                                <th><div>{{ __('Flag') }}</div></th>
                                <th><div>{{ __('Language') }}</div></th>
                                <th><div class="text-nowrap">{{ __('ISO Code') }}</div></th>
                                <th><div>{{ __('RTL') }}</div></th>
                                ...
                                <tr class="removable-item">
                                    ...
                                    <td>
                                        <div class="w-30 h-30 rounded-circle overflow-hidden">
                                            <img src="{{ $language->flag }}" class="w-100 h-100 object-fit-cover">
                                        </div>
                                    </td>
                                    <td>
                                        {{ $language->language }}
                                        @if ($language->default_language == ACTIVE)
                                            <span class="zBadge zBadge-active">{{ __('Default') }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-end align-items-center g-10">
                                            <a href="{{ route('admin.setting.language.edit', [$language->id, $language->iso_code]) }}" class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white" title="Edit">
                                            ...
                                            @if ($language->id != 1)
                                                <a href="javascript:void(0);"
                                                    data-url="{{ route('admin.setting.language.delete', [$language->id]) }}"
                                                    class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white delete" title="Delete">
                                            ...
                                            @endif
                                            <a href="{{ route('admin.setting.language.translate', [$language->id]) }}"
