...
@section('content')
    ...
    <div class="p-sm-30 p-15">
        <div class="d-flex align-items-center justify-content-between flex-wrap g-15 pb-26">
            <h2 class="fs-24 fw-600 lh-29 text-primary-dark-text">{{ __('Tax') }}</h2>
            <div class="">
                        ...
                        <table class="table zTable zTable-last-item-right commonDataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th><div class="text-nowrap">{{ __('Percentage') }} %</div></th>
                                    ...
                                    <tr class="removable-item">
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-end align-items-center g-10">
                                                <a class="p-0 bg-transparent w-30 h-30 bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center edit" data-item="{{ $tax }}"
                                                    data-updateurl="{{ route('admin.setting.tax.update', @$tax->id) }}"
                                                ...
                                                <button class="p-0 bg-transparent w-30 h-30 bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center deleteItem" data-formid="delete_row_form_{{$tax->id}}">
                                                ...
                                                <form action="{{ route('admin.setting.tax.delete', $tax->id) }}" method="post" id="delete_row_form_{{ $tax->id }}">
                                                    {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bd-c-stroke-color bd-ra-12 py-25 px-20">
                <div class="d-flex justify-content-between align-items-center g-10 pb-20 mb-20 bd-b-one bd-c-stroke">
                    <h5 class="fs-18 fw-500 lh-28 text-primary-dark-text">{{ __('Add Tax') }}</h5>
                ...
                <form action="{{ route('admin.setting.tax.store') }}" method="post" autocomplete="off">
                    @csrf
                    <div class="pb-24 rg-24 row">
                        <div class="col-12">
                        ...
                        <div class="col-12">
                            <label class="zForm-label" for="name">{{ __('Percentage') }}%</label>
                            <input class="zForm-control" type="number" name="percentage" step="any" id="percentage" placeholder="Enter percentage (1-100%)" min="1" max="100" required>
    ...
    <div class="modal fade edit_modal" id="add-todo-modal" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bd-c-stroke-color bd-ra-12 py-25 px-20">
                <div class="d-flex justify-content-between align-items-center g-10 pb-20 mb-20 bd-b-one bd-c-stroke">
                    <h5 class="fs-18 fw-500 lh-28 text-primary-dark-text">{{ __('Edit Tax') }}</h5>
                    <button type="button" class="w-30 h-30 rounded-circle d-flex justify-content-center align-items-center bd-one bd-c-stroke-color bg-transparent" data-bs-dismiss="modal" aria-label="Close"><i class="fa-solid fa-times"></i></button>
                </div>
                <form action="" id="updateEditModal" method="post">
                    ...
                    <div class="pb-24 rg-24 row">
                        ...
                        <div class="col-12">
                            <label class="zForm-label" for="name">{{ __('Percentage') }}%</label>
                            <input class="zForm-control" type="number" name="percentage" step="any" placeholder="Enter percentage (1-100%)" min="1" max="100" required>
                            @if ($errors->has('percentage'))
                                <span class="text-danger"><i class="fas fa-exclamation-triangle"></i>
                                    {{ $errors->first('percentage') }}</span>
                            @endif
...
@push('script')
    <script src="{{ asset('admin/js/custom/tax.js') }}"></script>
@endpush
