...
@section('content')
    ...
    <div class="p-sm-30 p-15">
        <div class="d-flex align-items-center justify-content-between flex-wrap g-15 pb-26">
            <h2 class="fs-24 fw-600 lh-29 text-primary-dark-text">{{ __('Blog Comment') }}</h2>
            <div class="">
                    ...
                    <tr class="removable-item">
                        ...
                        <td>
                            <div class="admin-dashboard-blog-list-img w-30 h-30 rounded-circle overflow-hidden">
                                <img src="{{asset(@$comment->customer->image)}}" class="w-100 h-100 object-fit-cover">
                            </div>
                        ...
                        <td>
                            <span id="hidden_id" class="d-none">{{$comment->id}}</span>
                            <div class="min-w-150 max-w-150">
                                <select name="status" class="status form-select">
                                    <option value="1" @if($comment->status == 1) selected @endif>{{ __('Published') }}</option>
                                    <option value="0" @if($comment->status != 1) selected @endif>{{ __('Unpublished') }}</option>
                                </select>
                            </div>
                        </td>
                        ...
                        <td>
                            <div class="action__buttons d-flex justify-content-center">
                                <button class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white deleteItem" data-formid="delete_row_form_{{ $comment->id }}">
                                ...
                                <form action="{{route('admin.blog.comment.delete', [$comment->id])}}" method="post" id="delete_row_form_{{ $comment->id }}">
                                    {{ method_field('DELETE') }}
...
@push('script')
    <script>
            "use strict"
            const blogCommentStatusRoute = "{{ route('admin.blog.comment.changeStatus') }}";
            const csrfToken= "{{ csrf_token() }}";
    </script>
    <script src="{{ asset('admin/js/custom/blog-comment.js') }}"></script>
@endpush
