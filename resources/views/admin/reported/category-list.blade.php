    ...
    <div class="p-sm-30 p-15">
        ...
        <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30 p-15">
            <table class="table zTable zTable-last-item-right commonDataTable">
                ...
                <tbody>
                @foreach($reportedCategories as $reportedCategory)
                    <tr class="removable-item">
                        <td>{{$loop->iteration}}</td>
                        <td>{{$reportedCategory->name}}</td>
                        <td>
                            @if($reportedCategory->status == ACTIVE)
                        ...
                        <td>
                            <div class="d-flex justify-content-end align-items-center g-10">
                                <a class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white edit" data-item="{{ $reportedCategory }}" data-updateurl="{{ route('admin.reported.category.update', @$reportedCategory->id) }}"
                                   data-toggle="tooltip"
                                   title="Edit">
                                ...
                                <button class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white deleteItem" data-formid="delete_row_form_{{$reportedCategory->id}}">
                                ...
                                <form action="{{ route('admin.reported.category.delete', $reportedCategory->id) }}" method="post" id="delete_row_form_{{ $reportedCategory->id }}">
                                    {{ method_field('DELETE') }}
    ...
    <div class="modal fade" id="add-todo-modal" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bd-c-stroke-color bd-ra-12 py-25 px-20">
                ...
                <form action="{{ route('admin.reported.category.store') }}" method="post">
                    @csrf
