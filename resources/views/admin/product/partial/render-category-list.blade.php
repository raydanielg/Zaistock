<table class="table zTable zTable-last-item-right commonDataTable">
    ...
    @foreach ($categories as $category)
        <tr class="removable-item">
            ...
            <td>
                <span id="hidden_id" class="d-none">{{$category->id}}</span>
                <div class="min-w-150 max-w-150">
                    <select name="status" class="status form-select">
                        <option value="1" {{ $category->status == ACTIVE ? 'selected':'' }} >{{ __('Active') }}</option>
                        <option value="0" {{ $category->status == DISABLE ? 'selected':'' }} >{{ __('Disable') }}</option>
                    </select>
                </div>
            </td>
            <td>
                <div class="d-flex justify-content-end align-items-center g-10">
                    <a class="p-0 bg-transparent w-30 h-30 bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center edit" data-item="{{ $category }}"
                       data-updateurl="{{ route('admin.product.category.update', @$category->uuid) }}"
                    ...
                    <button class="p-0 bg-transparent w-30 h-30 bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center deleteItem" data-formid="delete_row_form_{{ $category->uuid }}">
                    ...
                    <form action="{{ route('admin.product.category.delete', [$category->uuid]) }}" method="post" id="delete_row_form_{{ $category->uuid }}">
                        {{ method_field('DELETE') }}
