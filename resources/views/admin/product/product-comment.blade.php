    ...
    <div class="p-sm-30 p-15">
        ...
        <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30 p-15">
            <table class="table zTable zTable-last-item-right commonDataTable">
                ...
                <tr>
                    <th><div>{{ __('Sl') }}</div></th>
                    <th><div class="text-nowrap">{{ __('Commented By') }}</div></th>
                    <th><div>{{ __('Product') }}</div></th>
                    <th><div>{{ __('Comment') }}</div></th>
                ...
                @foreach ($productComments as $productComment)
                    <tr class="removable-item">
                        ...
                        <td> {{ @$productComment->customer->name }} </td>
                        <td>{{ @$productComment->product->title }}</td>
                        <td>{{ @$productComment->comment }}</td>
                        <td>{{ date('d M, Y', strtotime($productComment->created_at)) }}</td>
                        <td>
                            <div class="d-flex justify-content-start align-items-center g-10">
                                <div>
                                    @if($productComment->status == ACTIVE)
                                ...
                                <div>
                                    <ul class="action-list">
                                        <li class="nav-item dropdown">
                                            <a class="nav-link w-30 h-30 d-flex justify-content-center align-items-center bd-one bd-c-stroke rounded-circle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                <i class="fa-sharp fa-solid fa-ellipsis-vertical"></i>
                                            </a>
                                            <span id="hidden_id" class="d-none">{{$productComment->id}}</span>
                                            <ul class="dropdown-menu">
                                                @if($productComment->status != ACTIVE)
                                                    <li><a class="status dropdown-item" data-status="{{ ACTIVE }}" >{{ __('Active') }}</a></li>
                                                @endif
                                                @if($productComment->status != DISABLE)
                                                    <li><a class="status dropdown-item" data-status="{{ DISABLE }}" >{{ __('Disable') }}</a></li>
                                                @endif
                                            </ul>
                                        </li>
                                    </ul>
                        ...
                        <td>
                            <div class="d-flex align-items-center cg-5 justify-content-end">
                                <button class="d-flex justify-content-center align-items-center w-30 h-30 rounded-circle bd-one bd-c-stroke bg-white deleteItem" data-formid="delete_row_form_{{$productComment->id}}">
                                ...
                                <form action="{{ route('admin.product.comment.delete', $productComment->id) }}" method="post" id="delete_row_form_{{ $productComment->id }}">
                                    {{ method_field('DELETE') }}
...
@push('script')
    <script>
        'use strict'
        const productCommentStatusChangeRoute =  "{{ route('admin.product.comment.changeStatus') }}";
        const csrfToken = "{{ csrf_token() }}";
    </script>
    <script src="{{ asset('admin/js/custom/product-comment.js') }}"></script>
@endpush
